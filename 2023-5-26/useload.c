 ...
 * First column gives address of the ptr-to-func variable.
 * Second column gives the name of the corresponding function.
 * The lookup will use the string value (the function name) in the second
 * column as input and fill the pointer value in the first column.
 * */
...
#else
char SHLIBNAME[64] = "libtst1.so"; /* shared library in linux */
#endif
...
int main(int argc, char **argv)
{
	...
	SHLIBHDL hdl; /* Shared library handle */
 ...
 * INPUT and OUTPUT parameter: fptbl - array of function pointers and function names.
 */
...
SHLIBHDL load_shlib(char *libname, FUNCPTRELE *fptbl)
{
	...
	{
#if WINDOWS
		if ((*(fptbl->funcPtr) = GetProcAddress(hdl, fptbl->funcName)) == (FARPROC)NULL)
		{
